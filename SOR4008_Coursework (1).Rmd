...
```{r}
x = c(3.2, 7.8, 1.5, 9.0, 4.4, 2.1, 6.7, 0.3, 8.6, 5.9, 7.1, 3.8, 9.4, 2.7, 6.2, 1.9, 4.8, 0.6, 5.3, 8.1,
...
proposalfunction = function(params) {
    ...
    delta <- params[3]
    tau <- params[4]  
    new_alpha = rnorm(1, alpha, sd = 0.1)  # Adjust the sd as needed
    new_beta = rnorm(1, beta, sd = 0.1)
    new_delta = rnorm(1,delta, sd= 0.1)
...
posterior = tau^(n/2 + a - 1) * exp( -tau/2 sum[y-alpha-beta(xi)-delta(xi^2)] - b tau - 1/2((alpha/Sa)^2 + (beta/Sb)^2 + (delta/Sc)^2))
...
log_posterior <- function(params) {
  ...
  beta <- params[2]
  delta <- params[3]
  tau <- exp(params[4])  # log(tau) to ensure positive tau
  ...
  tau_prior <- dgamma(tau, shape = taushape, scale = tauscale, log = TRUE) + params[4]  # Adjust for log transformation
...
result <- metrop(log_posterior, initial = init, nbatch = 1000000, scale = c(0.1, 0.1, 0.1, 0.5))
})
...
calculate_MSE_MCMC <- function(y, x, result) {
  ...
  tau_samples <- exp(chain[, 4])  # Exponentiate tau to get the original scale
...
plot(exp(result$batch[, 4]), type = "l", main = expression(tau), ylab = "tau")
...
hist(exp(result$batch[, 4]), main = expression(tau), xlab = "tau")
